...
class SurfaceOutput:
    ...
    - context: volitelny kontext sceny / konverzace (pro budouci rozsireni)
    """
    text: str
    next_token_probs: Optional[List[float]] = None
    context: Optional[str] = None  # Přidáno z prototypu mirror.py
...
class MirrorConfig:
    """
    Konfiguracni trida pro snadne pouziti zrcadla.
    Kombinuje profil pozorovatele a zrcadlove osy.
    (Pridano pro kompatibilitu s prototypem mirror.py)
    """
...
class RozsirenaReflexe:
    ...
    # Pridano pro pristup k percepcnimu vektoru
    vektor: Optional[PercepcniVektor] = None
    parafraze: Optional[str] = None  # alias pro parafraze_text
    popis: Optional[str] = None      # alias pro popis_text
    lambda_pouzite: Optional[Dict[str, float]] = None
...
def interpolate(a: float, b: float, lam: float) -> float:
    """
    Lineární interpolace mezi hodnotami a a b podle parametru λ.
    
    Args:
        a: Počáteční hodnota (při λ=0)
        b: Cílová hodnota (při λ=1)
        lam: Interpolační koeficient ∈ [0,1]
    
    Returns:
        Interpolovaná hodnota: (1-λ)*a + λ*b
    
    (Přidáno z prototypu mirror.py pro čitelnost M_λ transformací)
    """
...
def mirror_analyzuj(
    ...
    return RozsirenaReflexe(
        ...
        # Nová pole pro kompatibilitu
        vektor=vektor1,
        parafraze=parafraze,
        popis=popis,
        lambda_pouzite={"deixis": lam_deixis, "styl": lam_styl},
    )


def mirror_f(surface: SurfaceOutput, cfg: MirrorConfig) -> RozsirenaReflexe:
    """
    Zkrácená verze mirror_analyzuj kompatibilní s prototypem mirror.py.
    
    Odpovídá teoretické funkci f(O_t; u, C, λ) → R_t
    
    Args:
        surface: SurfaceOutput (povrch/výstup modelu)
        cfg: MirrorConfig (konfigurace s profilem pozorovatele a osami)
    
    Returns:
        RozsirenaReflexe
    """
    return mirror_analyzuj(
        povrch=surface,
        osy=cfg.axis,
        jmeno_pozorovatele=cfg.observer.jmeno,
        rezim="agent",
        profil=cfg.observer,
    )
...
if __name__ == "__main__":
    ...
    # Test mirror_f zkratky
    print("\n=== TEST mirror_f() ===")
    cfg = MirrorConfig(
        observer=vytvor_defaultni_profil(),
        axis=demo_osy
    )
    r2 = mirror_f(demo_povrch, cfg)
    print(f"Formalita: {r2.vektor.formalita:.3f}")
    print(f"Vřelost: {r2.vektor.vrelost:.3f}")
    print(f"Asertivita: {r2.vektor.asertivita:.3f}")
